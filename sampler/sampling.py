...
class FrameSampler:
    ...
    # MAIN LOGIC
    # ------------------------------------------------------
    # def should_run_detector(self,tracker,tracks):
    #     self.frame_idx += 1
    #     frames_since_detect = self.frame_idx - self.last_detect_frame
    #     # trigger detection if there are unconfirmed tracks
    #     num_unconfirmed = sum(not t.is_activated for t in tracker.tracked_stracks)

    #     if num_unconfirmed > 0:
    #         return True , "unconfirmed_tracks"

    #     if frames_since_detect < self.min_gap:
    #         return False , f"cooldown({frames_since_detect})"
    #     # count active tracks
    #     # num_active = sum(t.is_activated for t in tracker.tracked_stracks)
    #     # num_active = sum(
    #     #     t.is_activated and t.state == TrackState.Tracked
    #     #     for t in tracker.tracked_stracks
    #     # )
    #     # active = {t[4] for t in tracks if t[6]==0}
    #     active = {t.track_id for t in tracks if t.is_activated}
    #     # if everything died, refresh quickly
    #     if len(active) == 0 and frames_since_detect >= self.max_gap // 2:
    ...
    def should_run_detector(self, tracker):
        self.frame_idx += 1
        frames_since_detect = self.frame_idx - self.last_detect_frame

        # Unconfirmed tracks need immediate follow-up
        num_unconfirmed = sum(not t.is_activated for t in tracker.tracked_stracks)
        if num_unconfirmed > 0:
            return True, "unconfirmed_tracks"
        ...
        num_active = len(tracker.tracked_stracks) + len(tracker.lost_stracks)

        if num_active == 0 and frames_since_detect >= self.max_gap // 2:
            return True, "no_active_tracks"

        # -------- uncertainty check --------
        U = self.compute_tracker_uncertainty(tracker)

        if U > 0.8:
            return True, f"tracker_uncertain({U:.2f})"
    ...
    # def compute_tracker_uncertainty(self, tracker):
    ...
    #         tsu = tracker.frame_id - t.frame_id
    #         tsu_score = min(tsu / 5.0, 1.0)

    #         # covariance uncertainty
    #         if t.covariance is not None:
    #             cov_score = min(np.trace(t.covariance) / 50.0, 1.0)
    #         else:
    #             cov_score = 0.0

    #         # detection confidence
    #         score_uncertainty = 1.0 - t.score

    #         # combine
    #         U_track = 0.4 * tsu_score + 0.4 * cov_score + 0.2 * score_uncertainty
    #         uncertainties.append(U_track)
    ...
    def compute_tracker_uncertainty(self, tracker):
        ...
        for t in tracker.tracked_stracks+tracker.lost_stracks:
            ...
            age = tracker.frame_id - t.start_frame
            age_score = min(age / 30.0, 1.0)

            # covariance growth
            if t.covariance is not None:
                cov_score = min(np.trace(t.covariance) / 10.0, 1.0)
            else:
                cov_score = 0.0
            ...
            U_track = 0.4 * age_score + 0.4 * cov_score + 0.2 * score_uncertainty
            uncertainties.append(U_track)
